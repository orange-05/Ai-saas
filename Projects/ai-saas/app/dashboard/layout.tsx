...
const tools = [
  ...
  { href: '/dashboard/conversation', icon: '💬', label: 'Conversation', color: '#6366f1' },
  { href: '/dashboard/image', icon: '🖼️', label: 'Image Generation', color: '#8b5cf6' },
  { href: '/dashboard/code', icon: '💻', label: 'Code Generation', color: '#06b6d4' },
  { href: '/dashboard/music', icon: '🎵', label: 'Music Generation', color: '#10b981', pro: true },
  { href: '/dashboard/video', icon: '🎬', label: 'Video Generation', color: '#f59e0b', pro: true },
];
    ...
    <div style={{ display: 'flex', minHeight: '100vh', background: '#111827' }}>
      ...
      <aside style={{
        ...
        <nav style={{ padding: '16px 12px', flex: 1 }}>
          <p style={{ color: '#374151', fontSize: '0.7rem', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.1em', padding: '0 8px', marginBottom: 8 }}>
            Tools
          </p>
          {tools.map((tool) => {
            const isActive = tool.exact ? pathname === tool.href : pathname.startsWith(tool.href);
            return (
              <Link key={tool.href} href={tool.href} className={`sidebar-link ${isActive ? 'active' : ''}`}>
                ...
                {tool.pro && (
                  <span style={{
                    ...
                    color: '#fff', padding: '2px 6px', borderRadius: 6, letterSpacing: '0.05em'
                  }}>PRO</span>
                )}
              </Link>
            );
        ...
        <div style={{ padding: '16px 12px' }}>
          <div className="upgrade-box">
            ...
            <Link href="/dashboard/settings" className="btn-primary" style={{ width: '100%', display: 'block', textAlign: 'center', padding: '10px 16px', fontSize: '0.85rem' }}>
              Upgrade Now
            </Link>
          </div>
        </div>
